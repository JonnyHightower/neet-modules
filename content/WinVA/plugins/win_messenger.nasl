# DESC  : Messenger Service Buffer Overrun
# AUTHOR: Fizz
# DATE  : 04/03/2006
#
# NOTES : This vulnerability check is based on the
#         messenger_03-043.nasl script provided from the nessus project.
#
...
function check_NT2K()
{
    ...
    if(soc)
    {
        ...
        len = strlen(recv);
        # If len = 32 then Windows NT Found
        # Else Windows 2K Found
	    close(soc);
	    return (1);
	}
	else
...
function check_rpc_serv()
{
    ...
    for(i=0;i<3;i++)
    {
        ...
        if(rep)
        {
            ...
            if("f7060000" >< hexstr(code) || "0300011c" >< hexstr(code))
            {
                display("Success");
                exit(0);
            }
