# Modified by Jon Roach to work with neet
  ...
  if (soc)
  {
   ...
   if (!isnull(list))
   {
    ...
    if (!isnull(nbname) && !isnull(domain))
    {
     ...
     report += string(" ", nbname, crap(data:" ", length:16 - strlen(nbname))," = ",unique_desc[0x00],"\n");
     report += string(" ", domain, crap(data:" ", length:16 - strlen(domain))," = ",group_desc[0x00],"\n");
     
     report = string (desc_445["english"],
		"\n\nPlugin output :\n\n",
		report);
